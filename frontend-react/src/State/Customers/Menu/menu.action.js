...
export const createMenuItem = ({menu,jwt}) => {
  return async (dispatch) => {
    ...
    try {
      const { data } = await admin_api.post("api/admin/food", menu,
      {
        headers: {
...
export const searchMenuItem = ({keyword,jwt}) => {
  return async (dispatch) => {
    ...
    try {
      const { data } = await admin_api.get(`api/food/search?name=${keyword}`,{
        headers: {
          Authorization: `Bearer ${jwt}`,
        },
      });
...
export const updateMenuItemsAvailability = ({foodId,jwt}) => {
  return async (dispatch) => {
    ...
    try {
      const { data } = await admin_api.put(`/api/admin/food/${foodId}`, {},{
        headers: {
          Authorization: `Bearer ${jwt}`,
        },
      });
...
export const updateFoodItem = ({foodId, title, price, jwt}) => {
  return async (dispatch) => {
    ...
    try {
      const { data } = await admin_api.put(`/api/admin/food/${foodId}`, { name: title, price: price }, {
        headers: {
          Authorization: `Bearer ${jwt}`,
        },
      });
...
export const deleteFoodAction = ({foodId,jwt}) => async (dispatch) => {
  ...
  try {
    const { data } = await admin_api.delete(`/api/admin/food/${foodId}`, {
      headers: {
        Authorization: `Bearer ${jwt}`,
      },
    });
